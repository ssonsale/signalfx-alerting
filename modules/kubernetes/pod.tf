resource "signalfx_detector" "cic-test-k8s_pods_active" {
  name         = "${var.sfx_prefix} K8S Pods active"
  description  = "Alerts when number of active pods changed significantly"
  program_text = <<-EOF
    from signalfx.detectors.against_recent import against_recent
    A = data('container_cpu_utilization', filter=filter('kubernetes_cluster', 'cic20-stg') and filter('kubernetes_namespace', '*') and filter('deployment', '*', match_missing=True) and filter('sf_tags', '*', match_missing=True)).sum(by=['kubernetes_cluster', 'kubernetes_namespace', 'kubernetes_pod_uid']).count().publish(label='A', enable=False)
...
resource "signalfx_detector" "cic-test-k8s_pods_failed_pending_ratio" {
  ...
  program_text = <<-EOF
    from signalfx.detectors.against_recent import against_recent
    A = data('kubernetes.pod_phase', filter=filter('kubernetes_cluster', 'cic20-stg') and filter('kubernetes_namespace', '*') and filter('metric_source', 'kubernetes')).below(1, inclusive=True).count(by=['kubernetes_cluster', 'kubernetes_namespace']).publish(label='A')
