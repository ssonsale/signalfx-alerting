/*
resource "signalfx_detector" "cic-test-k8s_node_cpu_imbalance" {
  name         = "${var.sfx_prefix} K8S Cluster CPU balance"
  description  = "Alerts when cluster CPU usage is imbalanced"
  program_text = <<-EOF
    A = data('container_cpu_utilization', filter=filter('kubernetes_cluster', 'cic20-stg') and filter('kubernetes_node', '*'), rollup='rate').sum(by=['kubernetes_node', 'kubernetes_cluster']).publish(label='A', enable=False)
    B = data('container_cpu_utilization', filter=filter('kubernetes_cluster', 'cic20-stg') and filter('kubernetes_node', '*')).sum(by=['kubernetes_node']).mean(by=['kubernetes_cluster']).publish(label='B', enable=False)
  ...
  rule {
    ...
    severity     = "Critical"
    disabled     = true
    notifications = ["Email,cloud-support@tibco.com"]
    parameterized_body  = var.message_body
  }
}
*/

resource "signalfx_detector" "cic-test-k8s_node_not_ready" {
...
resource "signalfx_detector" "cic-test-k8s_node_high_memory" {
  name         = "${var.sfx_prefix} K8S Node Memory > 90%"
  description  = "Alerts when K8s Node is using memory > 90% for 5m"
  program_text = <<-EOF
    A = data('memory.utilization', filter=filter('kubernetes_cluster', 'cic20-stg')).sum(by=['host', 'kubernetes_cluster']).publish(label='A')
